...
def get_ai_prediction(data_idx: int):
    ...
    try:
        ...
        val_errors = llm_providers.validate_provider_config(provider_name, CONFIG_DATA)
        if val_errors:
...
def get_live_prediction(data_idx: int, role: str = "model_a"):
    ...
    try:
        ...
        val_errors = llm_providers.validate_per_model_config(role, CONFIG_DATA)
        if val_errors:
        ...
        prov_errors = llm_providers.validate_provider_config(provider_name, CONFIG_DATA)
        if prov_errors:
        ...
        predictions = provider.predict(
            text,
            CONFIG_DATA.get('sentiment_elements', ["aspect_term", "aspect_category", "sentiment_polarity", "opinion_term"]),
            examples,
            aspect_categories,
            CONFIG_DATA.get('sentiment_polarity_options', ["positive", "negative", "neutral"]),
